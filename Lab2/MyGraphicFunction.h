...
public:
    ...
    double calculate(double x, std::stack<std::string> rpn)
    {
        ...
        while (!buffer.empty())
        {
            ...
            else
            {
                ...
                {
                    if (results.size() == 0)
                        throw "Wrong operator or operand format! (Оператор + стоит неправильно, нет первого слагаемого)\n";
                    double s = results.top();
                    results.pop();
                    if (results.size() == 0)
                        throw "Wrong operator or operand format! (Оператор + стоит неправильно, нет второго слагаемого)\n";
                    double f = results.top();
                    results.pop();
                    results.push(f + s);
                    buffer.pop();
                    break;
                ...
                {
                    if (results.size() == 0)
                        throw "Wrong operator or operand format! (Оператор + стоит неправильно, нет числа, которое нужно преобразовать)\n";
                    double f = results.top();
                    results.pop();
    ...
    void drawFunction(sf::RenderWindow& window, std::stack<std::string>& out) {
        ...
        int xc = luw + w / 2; // начало координат по X
        int yc = luh + h / 2; // начало координат по Y
        
        // Переменные для хранения предыдущей точки
        ...
        for (int i = 0; i < mass; ++i) {
            ...
            // Пропускаем точки с ошибкой (деление на ноль и т.д.)
            if (y == -1000000) {
                hasPrevPoint = false; // обрываем линию
                continue;
            }
            ...
            // Проверка границ
            bool inside = ((x1 - 1) >= luw && (x1 - 1) <= (luw + w) && 
                        (y1 - 1) >= luh && (y1 - 1) <= (luh + h));
            
            if (!inside) {
                hasPrevPoint = false; // обрываем линию при выходе за границы
                continue;
            }
            ...
            if (hasPrevPoint) {
                ...
                line[0] = sf::Vertex(sf::Vector2f(prevX, prevY), color);
                line[1] = sf::Vertex(sf::Vector2f(x1 - 1, y1 - 1), color);
                window.draw(line);
            }
            
            // Сохраняем текущую точку как предыдущую для следующей итерации
            prevX = x1 - 1;
            prevY = y1 - 1;
            hasPrevPoint = true;
        }
    ...
    void drawTangents(double coords, sf::RenderWindow &window, std::stack<std::string> &out, std::stack<std::string> &der, sf::Font &font) {
        ...
        sf::Color tangentColor = sf::Color(156, 156, 156);
        ...
        for (int i = 0; i < mass; ++i)
        {
            ...
            {
                sf::VertexArray line(sf::Lines, 2);
                line[0] = sf::Vertex(sf::Vector2f(prevX, prevY), tangentColor);
                line[1] = sf::Vertex(sf::Vector2f(x1 - 1, y1 - 1), tangentColor);
                window.draw(line);
            }
